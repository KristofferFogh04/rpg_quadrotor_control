...
AutoPilot<Tcontroller, Tparams>::AutoPilot(const ros::NodeHandle& nh,
    ...
    : nh_(nh),
      ...
      pose_estimate_available_(false),
      velocity_estimate_available_(false),
...

{
  ...
  control_command_pub_ =
      nh_.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", 1);
  ...
  pose_estimate_sub_ =
      nh_.subscribe("mavros/local_position/pose", 1,
                    &AutoPilot<Tcontroller, Tparams>::poseEstimateCallback,
                    this, ros::TransportHints().tcpNoDelay());
  velocity_estimate_sub_ =
      nh_.subscribe("mavros/local_position/velocity_local", 1,
                    &AutoPilot<Tcontroller, Tparams>::velocityEstimateCallback,
                    this, ros::TransportHints().tcpNoDelay());
...
template <typename Tcontroller, typename Tparams>
void AutoPilot<Tcontroller, Tparams>::cmdLoop(){
  ...
  temp_state_estimate_msg.header.frame_id = "local";

  received_state_est_ = quadrotor_common::QuadStateEstimate(temp_state_estimate_msg);
  ...
  //if (autopilot_state_ != States::OFF) {
    ...
    //ROS_INFO("[%d] State estimate", (predicted_state.position[0]));
  //}
...
template <typename Tcontroller, typename Tparams>
void AutoPilot<Tcontroller, Tparams>::velocityEstimateCallback(
  ...
  temp_vel_estimate_ = *msg;
  velocity_estimate_available_ = true;
  if (pose_estimate_available_ == true)
  {
    velocity_estimate_available_ = false;
    pose_estimate_available_ = false;
    cmdLoop();
...
template <typename Tcontroller, typename Tparams>
void AutoPilot<Tcontroller, Tparams>::poseEstimateCallback(
    const geometry_msgs::PoseStamped::ConstPtr& msg) {
  if (destructor_invoked_) {
    return;
  }

  temp_pose_estimate_ = *msg;
  pose_estimate_available_ = true;
  if (velocity_estimate_available_ == true)
  {
    velocity_estimate_available_ = false;
    pose_estimate_available_ = false;
    cmdLoop();
  ...
  if (autopilot_state_ == States::OFF) {
    if (state_estimate_available_) {
      if (received_state_est_.coordinate_frame ==
        ...
        std_msgs::Header start_msg;
        start_msg.frame_id = "arm";
        start_msg.seq = 1;
        px4_keepalive_pub.publish(start_msg);
      } else if (
          received_state_est_.coordinate_frame ==
...
quadrotor_common::ControlCommand AutoPilot<Tcontroller, Tparams>::land(
  ...
  if (command.collective_thrust <= 0.1) {
    setAutoPilotState(States::OFF);
    command.zero();
  }
